...
func (t Transaction) getBytesForSig(lockBytes, redeemBytes []byte, inType, sigType byte, index int, amount uint64, SegwitON bool) ([]byte, error) {
	...
	if inType == TypeP2PKH {
		if sigType == SigHashAll {
			...
			sigBytes, err = t.encodeToBytesV4(index, lockBytes, amount)

			/////multi chainn inputs
			//inputs := make([]mutxo.Input, 0)
			//
			//for _, vout := range t.Vouts {
			//
			//
			//	receipt := mutxo.Recipient{To: address.Address(vout.toBytes()), Value: pack.NewU256FromUint64(uint64(amount.IntPart()))}
			//
			//	recipients = append(recipients, receipt)
			//}
...
func (t Transaction) getHashesForSig(unlockData []TxUnlock, SegwitON bool, addressPrefix AddressPrefix) ([]TxHash, error) {
	...
	for i := 0; i < len(unlockData); i++ {
		...
		txHash, err := newTxHash(sigBytes, lockBytes, redeemBytes, inType, unlockData[i].SigType, addressPrefix)
		if err != nil {
			return nil, err
		}
