...
func Test_Byte_Type(t *testing.T) {
	...
	//binarySerializer.PutUint32(&buf, binary.LittleEndian, uint32(4)|(1<<31))

	fmt.Printf("version:%x\n", buf.Bytes())
	SigHashAll := 0x1
	SigHashAnyOneCanPay := 0x80

	bb := SigHashAll&SigHashAnyOneCanPay == 0
	...
	//CurveOrder     = []byte{0xFF, 0xFF}

	//	hash := id.Hash(CurveOrder)

	branchId := []byte{0xA6, 0x75, 0xff, 0xe9}

	fmt.Printf("%x \n", branchId)

	bid := uint32ToLittleEndianBytes(consensusBranchId)
