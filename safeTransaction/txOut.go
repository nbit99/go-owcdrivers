...
func newTxOutForEmptyTrans(vout []Vout, addressPrefix AddressPrefix) ([]TxOut, error) {
	...
	unlockHeight := uint64ToLittleEndianBytes(0)

	reserveBytes := []byte("safe")
	...
	for _, v := range vout {
		...
		if strings.Index(v.Address, prefixStr) == 0 {
			...
			ret = append(ret, TxOut{amount, redeem, unlockHeight, -10, reserveBytes})
		...
		ret = append(ret, TxOut{amount, hash, unlockHeight, -10, reserveBytes})
	}
	return ret, nil
}
...
func (out TxOut) toBytes() ([]byte, error) {
	...
	if DefaultTxVersion > SAFE_TX_VERSION_1 {
		ret = append(ret, out.unlockedHeight...)
		ret = append(ret, byte(len(out.vReserve)))
		ret = append(ret, out.vReserve...)
	}
