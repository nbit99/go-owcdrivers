...
func DecodeRawTransaction(txBytes []byte, SegwitON bool) (*Transaction, error) {
	...
	for i := byte(0); i < numOfVouts; i++ {
		...
		if DefaultTxVersion > SAFE_TX_VERSION_1 {//101之后output添加字段 unlockedHeight reserve
			if index+8 > limit {
				return nil, errors.New("Invalid transaction data length!")
			}
			tmpTxOut.unlockedHeight = txBytes[index : index+8]
			index += 8

			//reserve
			if index+1 > limit {
				return nil, errors.New("Invalid transaction data length, vReserve!")
			}
			reserveLen := txBytes[index]
			index++
			...
			if index+int(reserveLen) > limit {
				return nil, errors.New("Invalid transaction data length!")
			}
			tmpTxOut.vReserve = txBytes[index : index+int(reserveLen)]
			index += int(reserveLen)
		}
