...
func Encode(prefix, alphabet string, payload []byte) string {
	...
	ecashExpandPrefix := getPrefix(prefix)

	checksum = calcChecksum(ecashExpandPrefix, extendPayload)
...
func getPrefix(prefix string) []int8 {
	encodePrefix := EncodePrefix(prefix)
	ecashExpandPrefix := make([]int8, len(encodePrefix))
	for i := 0; i < len(encodePrefix); i++ {
		ecashExpandPrefix[i] = int8(encodePrefix[i])
	}

	return ecashExpandPrefix
}

func Decode(address, prefix string) ([]byte, error) {
	lower := false
	upper := false
	hasNumber := false
	prefixSize := 0
	...
	ecashExpandPrefix := getPrefix(prefixStr)

	if !verifyChecksum(ecashExpandPrefix, value) {
		return nil, ErrorInvalidAddress
	}
